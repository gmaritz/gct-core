...
function findRate(value: unknown): { rate: HotelbedsRate; currency?: string } | undefined {
  const object = asObject(value);
  if (!object) return undefined;
  if (typeof object.rateKey === "string" || typeof object.net === "string" || typeof object.sellingRate === "string") {
    return { rate: object as HotelbedsRate, currency: typeof object.currency === "string" ? object.currency : undefined };
  }
...
function mapStatus(rate: HotelbedsRate, fallback: AccommodationAvailabilityRateStatus): AccommodationAvailabilityRateStatus {
  if (rate.rateType === "RECHECK") return "RECHECK_REQUIRED";
  if (typeof rate.allotment === "number" && rate.allotment <= 0) return "UNAVAILABLE";
  if (rate.rateType === "BOOKABLE" || typeof rate.allotment === "number") return "BOOKABLE";
  return fallback;
}

function childAges(rate: HotelbedsRate, fallback: ReadonlyArray<number>): ReadonlyArray<number> {
  if (!rate.childrenAges) return Object.freeze([...fallback]);
...
function mapRate(
  supplierRate: HotelbedsRate,
  selected: AccommodationRateOption,
...
export function mapHotelbedsCheckRateResponse(
  payload: unknown,
  selectedRate: AccommodationRateOption,
): AccommodationRateOption {
  const found = findRate(payload);
  if (!found) throw new Error("Hotelbeds CheckRate response did not contain a valid rate.");
