import { createHotelbedsIntegrationError, HotelbedsIntegrationErrorCode } from "../client";
import { HotelContentRecord } from "../../../content/models/hotel-content-record";
...
function parseStarRating(hotel: HotelbedsHotel): number | undefined {
...
function resolveLastUpdatedAt(hotel: HotelbedsHotel, fallback: Date): { lastUpdatedAt: Date; rawLastUpdate?: string } {
  const rawLastUpdate =
    hotel.images?.find((image) => Boolean(image.lastUpdate))?.lastUpdate;
...
export class HotelbedsContentMapper {
  public mapHotel(hotel: HotelbedsHotel, fallbackUpdatedAt: Date): HotelContentRecord {
