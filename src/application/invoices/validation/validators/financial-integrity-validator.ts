    ...
    if (invoice) {
      if (!isFiniteNumber(invoice.amountPaid) || invoice.amountPaid < 0) {
        errors.push(
          createInvoiceValidationError({
            stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
            code: InvoiceValidationErrorCode.INVALID_AMOUNT_PAID,
      ...
      if (!isFiniteNumber(invoice.refundableAmount) || invoice.refundableAmount < 0) {
        errors.push(
          createInvoiceValidationError({
            stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
            code: InvoiceValidationErrorCode.INVALID_REFUNDABLE_AMOUNT,
      ...
      if (invoice.depositRequirement) {
        ...
        if (!isValidType || !isValidValue || !percentageWithinRange) {
          errors.push(
            createInvoiceValidationError({
              stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
              code: InvoiceValidationErrorCode.INVALID_DEPOSIT_REQUIREMENT,
      ...
      for (const allocation of invoice.paymentAllocations) {
        ...
        if (allocationInvalid) {
          errors.push(
            createInvoiceValidationError({
              stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
              code: InvoiceValidationErrorCode.INVALID_PAYMENT_ALLOCATION,
      ...
      if (invoice.paymentAllocations.length > 0 && isFiniteNumber(invoice.amountPaid) && paymentAllocationTotal !== invoice.amountPaid) {
        errors.push(
          createInvoiceValidationError({
            stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
            code: InvoiceValidationErrorCode.PAYMENT_ALLOCATION_TOTAL_MISMATCH,
      ...
      for (const adjustment of invoice.adjustments) {
        ...
        if (invalidAdjustment) {
          errors.push(
            createInvoiceValidationError({
              stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
              code: InvoiceValidationErrorCode.INVALID_ADJUSTMENT,
      ...
      if (invoice.cancellationSnapshot) {
        ...
        const invalidSnapshot =
          isBlank(snapshot.policyReference)
        ...
        if (invalidSnapshot) {
          errors.push(
            createInvoiceValidationError({
              stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
              code: InvoiceValidationErrorCode.INVALID_CANCELLATION_SNAPSHOT,
      ...
      if (typeof invoice.dueDate !== "undefined" && !isValidDate(invoice.dueDate)) {
        errors.push(
          createInvoiceValidationError({
            stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
            code: InvoiceValidationErrorCode.INVALID_DUE_DATE,
      ...
      for (const reference of invoice.externalReferences) {
        if (isBlank(reference.system) || isBlank(reference.reference)) {
          errors.push(
            createInvoiceValidationError({
              stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
              code: InvoiceValidationErrorCode.INVALID_EXTERNAL_REFERENCE,
      ...
      if (invalidMetadata) {
        errors.push(
          createInvoiceValidationError({
            stage: InvoiceValidationStage.FINANCIAL_INTEGRITY,
            code: InvoiceValidationErrorCode.INVALID_METADATA,
